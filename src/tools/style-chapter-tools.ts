...
export function registerStyleChapterTools(server: McpServer): void {
  server.tool("create_style_extraction_template", { project_id: z.string().optional() }, async (input) => toolText({ project_id: input.project_id, ...createStyleExtractionTemplate() }));

  server.tool("submit_style_profile", { project_id: z.string(), profile: StyleProfileSchema }, async (input) => {
    const saved = await updateProject(input.project_id, (project) => ({ ...project, styleProfile: input.profile }));
    return toolText({ project_id: saved.id });
  });
  ...
  server.tool("create_chapter_extraction_template", { project_id: z.string().optional(), title: z.string().optional(), chapter_count: z.number().int().min(1).max(30).optional() }, async (input) => toolText({ project_id: input.project_id, outline: createChapterExtractionTemplate(input) }));
  ...
  server.tool("build_chapter_worldbook_entries", { project_id: z.string().optional(), outline: ChapterOutlineSchema.optional(), base_order: z.number().int().optional(), comment_prefix: z.string().optional() }, async (input) => {
    ...
    if (input.project_id) {
      await updateProject(input.project_id, (project) => ({ ...project, chapterOutline: outline }));
    }
