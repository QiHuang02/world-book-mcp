...
const RESOURCES: Record<string, string> = {
  "usage-guide": `# world-book-mcp 使用指南\n\n首次使用空白目录时先调用 init_project。kind=worldbook 会创建独立世界书模板；kind=character_card 或 mixed 会创建 chara_card_v3 模板，其中 mixed 适合角色卡 + 内嵌世界书一体化项目。推荐 agent 使用 upsert_worldbook_entry / upsert_worldbook_entries 写条目：只提交 comment、keys、content 等核心字段，MCP 自动补全完整世界书 entry 结构并校验。旧的全量 draft 提交逻辑已降级为内部/兼容层，不作为默认公开接口。\n\n同一 project 的写入会在 MCP 进程内串行化，并返回 revision；高并发 agent 可传 expected_revision 做冲突检测。patch apply 会先写入导出 JSON，再更新 project；若 project 更新失败会尽力恢复导出文件。patch match.uid 优先匹配导入时保留的 sourceUid；新建草稿建议使用 index 或 comment 定位。`,
