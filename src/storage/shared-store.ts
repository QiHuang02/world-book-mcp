...
import { draftSlicePath, readDraftSlice, upsertDraftSlice } from "./draft-store.js";
...
export function sharedCategoryForType(type: DraftType): SharedCategory {
  return type === "entry" ? "entries" : "assets";
}

function sharedDirForCategory(category: SharedCategory): string {
  return category === "entries" ? SHARED_ENTRIES_DIR : SHARED_ASSETS_DIR;
}

function sharedSlicePath(category: SharedCategory, id: string): string {
  const dir = sharedDirForCategory(category);
  return assertInside(dir, path.resolve(dir, `${sanitizeFilename(id)}.json`));
}

export async function ensureSharedDirs(): Promise<void> {
  await fs.mkdir(SHARED_ENTRIES_DIR, { recursive: true });
  await fs.mkdir(SHARED_ASSETS_DIR, { recursive: true });
}

export async function readSharedRegistry(): Promise<SharedRegistry> {
  await ensureSharedDirs();
  try {
    return await readJsonFile(SHARED_REGISTRY_PATH, SharedRegistrySchema);
  } catch (error) {
...
export async function writeSharedRegistry(registry: SharedRegistry): Promise<void> {
  await ensureSharedDirs();
  await writeJsonFile(SHARED_REGISTRY_PATH, SharedRegistrySchema.parse(registry));
}
...
export async function shareSlice(input: { slug: string; type: DraftType; id: string; sharedId?: string; title?: string; overwrite?: boolean }): Promise<{ entry: SharedRegistryEntry; path: string; slice: DraftSlice }> {
  ...
  const exists = registry.entries.some((entry) => entry.id === sharedId);
  if (exists && !input.overwrite) throw new Error(`共享切片 ${sharedId} 已存在；如需覆盖请设置 overwrite=true`);
  const sharedSlice = DraftSliceSchema.parse({ ...slice, id: sharedId, title: input.title ?? slice.title ?? slice.id, data: DraftSliceDataSchemas[slice.type].parse(slice.data), revision: 0, updatedAt: nowIso() });
  await writeJsonFile(outputPath, sharedSlice);
  const entry: SharedRegistryEntry = {
    id: sharedId,
    type: sharedSlice.type,
    category,
    title: sharedSlice.title ?? sharedId,
    source_project: input.slug,
    shared_at: nowIso(),
    file: path.relative(SHARED_DIR, outputPath).replace(/\\/g, "/"),
  };
...
export async function useShared(input: { slug: string; sharedId: string; targetId?: string; overwrite?: boolean }): Promise<{ registry_entry: SharedRegistryEntry; path: string; slice: DraftSlice }> {
  ...
  if (!input.overwrite) {
    try {
      await fs.access(draftSlicePath(input.slug, sharedSlice.type, targetId));
      throw new Error(`目标切片 ${sharedSlice.type}/${targetId} 已存在；如需覆盖请设置 overwrite=true`);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    }
  }
  const now = nowIso();
  const targetSlice = DraftSliceSchema.parse({ ...sharedSlice, id: targetId, createdAt: now, updatedAt: now, revision: 0 });
