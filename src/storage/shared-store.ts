...
export const SHARED_REGISTRY_PATH = path.resolve(SHARED_DIR, "registry.yaml");
...
function sharedSlicePath(category: SharedCategory, id: string): string { const dir = sharedDirForCategory(category); return assertInside(dir, path.resolve(dir, `${sanitizeFilename(id)}.yaml`)); }
...
export async function readSharedRegistry(): Promise<SharedRegistry> { await ensureSharedDirs(); try { return await readYamlFile(SHARED_REGISTRY_PATH, SharedRegistrySchema); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return { version: 4, entries: [] }; throw error; } }
export async function writeSharedRegistry(registry: SharedRegistry): Promise<void> { await ensureSharedDirs(); await writeYamlFile(SHARED_REGISTRY_PATH, SharedRegistrySchema.parse(registry)); }
...
export async function shareSlice(input: { slug: string; type: DraftType; id: string; sharedId?: string; title?: string; overwrite?: boolean }): Promise<{ entry: SharedRegistryEntry; path: string; slice: DraftSlice }> {
  ...
  await writeYamlFile(outputPath, sharedSlice);
...
export async function useShared(input: { slug: string; sharedId: string; targetId?: string; overwrite?: boolean }): Promise<{ registry_entry: SharedRegistryEntry; path: string; slice: DraftSlice }> {
  ...
  const sharedSlice = await readYamlFile(sourcePath, DraftSliceSchema);
...
export async function listShared(input: { type?: DraftType; category?: SharedCategory; includeContent?: boolean } = {}): Promise<Array<SharedRegistryEntry & { path: string; slice?: DraftSlice }>> {
  ...
  for (const entry of entries) {
    ...
    result.push({ ...entry, path: filePath, ...(input.includeContent ? { slice: await readYamlFile(filePath, DraftSliceSchema) } : {}) });
  }
  return result;
}
