...
export function validateCharacterCardConfig(input: {
  ...
  if (config.card.description.trim()) {
    ...
    issues.push(issue({ code: "character_card.description.not_empty", field: "card.description", severity, message: severity === "error" ? "description 内含大段人设且项目 draft 为空；请使用 entry slice 承载角色信息" : "当前规范建议 description 为空，角色信息放入世界书条目" }));
  }
  ...
  return withValid(section({ ...splitIssues(issues), summary: { entry_count: config.worldbook.source === "project_draft" ? draft?.length ?? 0 : 0, greeting_count: 1 + config.card.alternate_greetings.length, description_empty: !config.card.description.trim() } }));
}
