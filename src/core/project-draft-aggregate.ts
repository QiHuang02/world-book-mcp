...
import { mvuContentFromEntryRecords, MVU_ENTRY_ID_TO_KEY, type MvuContentView } from "./mvu-entry-templates.js";

export interface ProjectDraftAggregate {
  worldbookDraft: WorldbookDraftEntry[];
  characterCardConfig?: CharacterCardConfig;
  mvuConfig?: MvuConfig;
  mvuContent?: MvuContentView;
...
export function aggregateDraftSlices(project: Project, slices: DraftSlice[]): ProjectDraftAggregate {
  ...
  const mvuContent = mvuConfig ? mvuContentFromSlices(slices) : undefined;
  ...
  return { worldbookDraft, characterCardConfig, mvuConfig, mvuContent, htmlBeautifyConfig, ejsConfig, regexSlices };
}

export interface HydratedProject {
  project: Project & { draft?: WorldbookDraftEntry[]; characterCardConfig?: CharacterCardConfig; mvuConfig?: MvuConfig; mvuContent?: MvuContentView; htmlBeautifyConfig?: HtmlBeautifyConfig; ejsConfig?: EjsConfig; regexSlices?: ProjectDraftAggregate["regexSlices"] };
  slug: string;
...
export function projectWithAggregate(project: Project, aggregate: ProjectDraftAggregate): HydratedProject["project"] {
  return { ...project, draft: aggregate.worldbookDraft, characterCardConfig: aggregate.characterCardConfig, mvuConfig: aggregate.mvuConfig, mvuContent: aggregate.mvuContent, htmlBeautifyConfig: aggregate.htmlBeautifyConfig, ejsConfig: aggregate.ejsConfig, regexSlices: aggregate.regexSlices };
}

function mvuContentFromSlices(slices: DraftSlice[]): MvuContentView {
  return mvuContentFromEntryRecords(activeSlicesOf(slices, "entry").filter((slice) => MVU_ENTRY_ID_TO_KEY[slice.id]).map((slice) => ({ id: slice.id, entry: dataOf(slice, "entry") })));
}
