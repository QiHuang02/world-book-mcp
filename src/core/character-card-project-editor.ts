...
export const CharacterGreetingsChangesSchema = z.object({
  first_mes: z.string().optional(),
  alternate_greetings: z.array(z.string()).optional(),
}).strict().refine((value) => Object.keys(value).length > 0, { message: "changes 至少需要一个字段" });
...
export function defaultProjectProfile(projectName: string): ProjectProfile {
  return ProjectProfileSchema.parse({
    ...
    worldbook_name: `${projectName}世界书`,
  });
}

export function defaultProjectGreetings(project: Project): ProjectGreetings {
  return ProjectGreetingsSchema.parse({
    first_mes: project.greetings?.first_mes ?? project.profile?.first_mes ?? "",
    alternate_greetings: project.greetings?.alternate_greetings ?? project.profile?.alternate_greetings ?? [],
  });
}

export function applyCharacterProfileUpdate(project: Project, changes: CharacterProfileChanges): Project {
  const profile = ProjectProfileSchema.parse({
    ...(project.profile ?? defaultProjectProfile(project.name)),
    ...changes,
  });
  return { ...project, profile };
}

export function applyCharacterGreetingsUpdate(project: Project, changes: CharacterGreetingsChanges): Project {
  const greetings = ProjectGreetingsSchema.parse({
    ...defaultProjectGreetings(project),
    ...changes,
  });
  return { ...project, greetings };
}
