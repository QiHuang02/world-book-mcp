...
export function validateHtmlBeautifyConfig(input: { html: HtmlBeautifyConfig; mvu?: MvuConfig; characterCardConfig?: CharacterCardConfig }): HtmlBeautifyValidationResult {
  ...
  if (statusbarTarget && hasCdata(statusbarSource)) issues.push(normalizeIssue({ code: "html.statusbar.cdata", field: "statusbar.html", severity: "error", message: "HTML 状态栏不应包含 CDATA 包裹" }));
  if (statusbarTarget && hasBareStatDataMacro(statusbarSource)) issues.push(normalizeIssue({ code: "html.statusbar.bare_stat_data", field: "statusbar.html", severity: "error", message: "HTML 状态栏不应使用裸 {{stat_data...}}，请改用 {{format_message_variable::...}}" }));
