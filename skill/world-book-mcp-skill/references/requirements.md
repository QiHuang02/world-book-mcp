...
5. **互动与开场主题**：user 关系、初始场景、互动氛围、开场白数量。
6. **可选资产主题**：MVU/EJS/HTML。启用 MVU 时必须确认变量用途、变量树、默认值、更新条件、展示范围；EJS 必须确认已有 MVU。
...
| 需求结果 | MCP 写入 |
|---|---|
| 角色卡 profile | `update_character_profile`，`description` 默认空 |
